...
export function AccountDialog({ open, onOpenChange, accounts, onChanged }: AccountDialogProps) {
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal>
    ...
    <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 w-[min(420px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-background p-5 shadow-xl">
      ...
      <div className="mt-4 space-y-2">{accounts.map((account) => <div key={account.uuid} className="flex items-center justify-between rounded-md border border-border p-2 text-sm">
        <span>{account.displayName || account.username} <span className="text-muted-foreground">{account.type}</span></span>
        <div className="flex gap-1">{!account.selected && <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run(() => AccountService.SelectAccount(account.uuid))}>Use</Button>}{account.type === 'ely.by' && <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run(() => AccountService.LogoutElyBy(account.uuid))}>Log out</Button>}</div>
      </div>)}</div>
      <form className="mt-5 space-y-3 border-t border-border pt-4" onSubmit={addAccount}>
        ...
        <div className="flex justify-end gap-2"><Dialog.Close render={<Button type="button" variant="ghost" />}>Close</Dialog.Close><Button type="submit" disabled={busy || !username.trim() || (type === 'ely.by' && !password)}>Add account</Button></div>
      </form>
    </Dialog.Popup>
