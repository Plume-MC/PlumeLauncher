          ...
          <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 flex h-[min(680px,90vh)] w-[min(720px,92vw)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-lg border border-border bg-background shadow-xl">
            ...
            <Tabs value={tab} onValueChange={(value) => setTab(value as SettingsTab)} className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="min-h-0 flex-1 overflow-auto px-5 py-4 text-sm">
                ...
                <TabsContent value="data" className="flex flex-col gap-3"><p className="text-xs text-muted-foreground">The new data root is applied after restarting the launcher. Existing data is not moved.</p><label className="flex flex-col gap-1 text-xs">Data root<Input value={dataRoot} onChange={(event) => setDataRoot(event.target.value)} /></label><div className="flex gap-2"><Button size="sm" disabled={!dataRoot || !!busy} onClick={() => void saveDataRoot()}>{busy === 'data-root' ? 'Saving...' : 'Use after restart'}</Button><Button variant="secondary" size="sm" disabled={!!busy} onClick={() => void SystemService.OpenLogFolder()}>Open logs</Button></div></TabsContent>
